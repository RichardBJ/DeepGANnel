...
def define_generator(latent_dim, n_classes=4):
    ...
    # upsample to 14x14
    gen = Conv1DTranspose(192, 3, padding='same', kernel_initializer=init)(merge)
    gen = BatchNormalization()(gen)
    gen = Activation('relu')(gen)
    
    #upsample
    gen = Conv1DTranspose(96, 3, padding='same', kernel_initializer=init)(gen)
    gen = BatchNormalization()(gen)
    gen = Activation('relu')(gen)
...
def define_generator(latent_dim, n_classes=4):
    ...
    gen = Activation('relu')(gen)
    gen = Reshape((192, 1, 384))(gen)
    # merge image gen and label input
    merge = Concatenate()([gen, li])
    # upsample to 14x14
    gen = Conv2DTranspose(192, (3,3), strides=(2,1), padding='same', kernel_initializer=init)(merge)
    gen = BatchNormalization()(gen)
    gen = Activation('relu')(gen)
    
    #upsample
    gen = Conv2DTranspose(96, (3,3), strides=(2,1), padding='same', kernel_initializer=init)(gen)
    gen = BatchNormalization()(gen)
    gen = Activation('relu')(gen)
    ...
    # upsample to 28x28
    gen = Conv2DTranspose(1, (3,3), strides=(2,1), padding='same', kernel_initializer=init)(gen)
    out_layer = Activation('tanh')(gen)
    # define model
    model = Model([in_lat, in_label], out_layer)
    model.summary()
