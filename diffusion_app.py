...
Updates every few optimiser steps:
  - loss curve so far,
  - latest generated raw trace beside a real one,
  - all-points amplitude histogram of real vs generated current.

Streams by yielding from the training loop, so the browser updates as
training progresses.
...
from markov_fit import fit_markov_to_idealisation, dwell_summary
...
def _parse_open_states(open_states, n_states=None):
    """UI is 1-based to match the matrix row/column labels; internals are 0-based."""
    try:
        idx = {int(s) for s in str(open_states).replace(" ", "").split(",") if s != ""}
    except ValueError:
        raise gr.Error("Open states must be whole numbers, e.g. '3,4'.")
    if not idx:
        raise gr.Error("Specify at least one open-state index, e.g. '3,4'.")
    if n_states is not None and (min(idx) < 1 or max(idx) > n_states):
        raise gr.Error(f"Open-state indices must be between 1 and {n_states}.")
...
def _ideal_fig(ideal, open_set, n_channels, real=None, real_label=None, gen=None, gen_label=None):
    ...
    axes[0].set_title(
        f"Simulated idealisation (open states {sorted(i + 1 for i in open_set)}, "
...
def preview_ideal(matrix, open_states, n_channels, preview_len, folder, ckpt_path):
    ...
    open_set = _parse_open_states(open_states, P.shape[0])
...
def fit_matrix_to_record(matrix, open_states, folder, page_idx, fit_channels,
                         fit_samples, restarts, prior, progress=gr.Progress()):
    """
    Fit the transition matrix to the idealisation of one seed record.

    The topology is taken from the matrix as it currently stands: every zero
    entry is treated as a forbidden transition and stays zero, every non-zero
    entry is free. Edit the matrix to set the topology, then fit.
    """
    P0 = _normalise_rows(matrix)
    S = P0.shape[0]
    open_set = _parse_open_states(open_states, S)

    pages = load_pages(folder, verbose=False)
    if not pages:
        raise gr.Error(f"No readable pages in '{folder}'.")
    idx = int(page_idx) % len(pages)
    chan = np.rint(pages[idx][:, 0]).astype(np.int64)

    allowed = (np.asarray(matrix if not hasattr(matrix, "values") else matrix.values,
                          dtype=float) != 0).astype(float)
    n_ch = int(fit_channels) if int(fit_channels) > 0 else None

    progress(0.02, desc="Fitting")
    try:
        res = fit_markov_to_idealisation(
            chan, S, open_set, n_channels=n_ch, allowed=allowed,
            n_restarts=int(restarts), prior=float(prior),
            max_samples=int(fit_samples), P_init=P0, seed=0,
            progress=lambda f: progress(0.02 + 0.96 * f, desc="Fitting"),
        )
    except ValueError as e:
        raise gr.Error(str(e))

    P = res["P"]
    sim = simulate_markov_idealisation(
        P, open_set, n_channels=res["n_channels"],
        n_steps=min(len(chan), int(fit_samples)), seed=0,
    )
    real_d, fit_d = dwell_summary(chan[: len(sim)]), dwell_summary(sim)
    levels = sorted(set(real_d) | set(fit_d))
    dwells = ", ".join(
        f"{k}: {real_d.get(k, float('nan')):.1f}/{fit_d.get(k, float('nan')):.1f}"
        for k in levels
    )
    status = (
        f"Fitted page {idx} ({res['n_samples']} samples, {res['n_channels']} channel(s)). "
        f"log-likelihood {res['loglik']:.1f}, BIC {res['bic']:.1f}, "
        f"{res['n_iter']} iterations, {'converged' if res['converged'] else 'hit the iteration cap'}. "
        f"Restart log-likelihoods: {', '.join(f'{v:.1f}' for v in res['restart_logliks'])}. "
        f"Mean dwell real/fitted per level -- {dwells}. "
        "Matrix updated; hit Preview to eyeball it."
    )
    return np.round(P, 5).tolist(), status
...
with gr.Blocks(title="Ion diffusion trainer") as demo:
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            open_states = gr.Textbox("3,4", label="Open-state indices (1-based, comma-separated)")
            ...
            gr.Markdown(
                "**Fit to a seed record.** Zeros in the matrix above are held at "
                "zero, so edit it first to set the topology. The fit uses the "
                "exact sequence likelihood, not dwell histograms."
            )
            with gr.Row():
                fit_page = gr.Number(0, label="Seed page index", precision=0)
                fit_channels = gr.Number(0, label="Channels in record (0 = infer)", precision=0)
            with gr.Row():
                fit_samples = gr.Slider(2000, 200_000, value=50_000, step=1000,
                                        label="Samples to fit")
                restarts = gr.Slider(1, 10, value=3, step=1, label="Restarts")
            prior = gr.Slider(0.0, 5.0, value=0.5, step=0.1,
                              label="Dirichlet prior (0 = maximum likelihood)")
            fit_btn = gr.Button("Fit matrix to seed record")
        ...
        with gr.Column(scale=2):
            ...
            fit_status = gr.Textbox(label="Fit status", lines=3)
    ...
    fit_btn.click(
        ...
        inputs=[matrix, open_states, folder, fit_page, fit_channels,
                fit_samples, restarts, prior],
        outputs=[matrix, fit_status],
    )
