...
def run_training(
    ...
    # ============================================================
    if use_slow:
        cpages = decimate_pages(pages, factor, phases=int(slow_phases))
        max_clen = max((len(p) for p in cpages), default=0)
        ...
        Xc, Yc = build_dataset(cpages, seq_len=coarse_seq_len, stride=1)
        ccond_mean, ccond_std = float(Xc.mean()), float(Xc.std()) + 1e-8
        ...
        coarse = ConditionalDiffusion(
            ...
            timesteps=int(timesteps), cond_channels=1,
        ).to(device)
        copt = optim.AdamW(coarse.parameters(), lr=float(lr), weight_decay=1e-4)
        ...
        with torch.no_grad():
            ...
            cide0 = (block_mean(chan0, factor) - ccond_mean) / ccond_std
        ...
        coarse_blob = {
            ...
            "config": {"base_channels": int(base_channels), "time_dim": 128,
                       "timesteps": int(timesteps), "cond_channels": 1},
            "norm": {"cond_mean": ccond_mean, "cond_std": ccond_std,
    ...
    write_run_metadata(
        ...
        "Conditional diffusion training run",
        {
            ...
            "Coarse norm": (
                "cond_mean={cond_mean:.6g}, cond_std={cond_std:.6g}, "
                "cur_mean={cur_mean:.6g}, cur_std={cur_std:.6g}".format(**coarse_blob["norm"])
...
def _ideal_fig(ideal, open_set, n_channels):
    fig, ax = plt.subplots(figsize=(6, 2.4))
    ax.plot(ideal, color="tab:gray", drawstyle="steps-post")
    ax.set_yticks(range(int(n_channels) + 1))
    ax.set_xlabel("sample")
    ax.set_ylabel("open channels")
    ax.set_title(
        f"Idealisation preview  (open states {sorted(open_set)}, "
...
def preview_ideal(matrix, open_states, n_channels, preview_len):
    plt.close("all")
    ...
    return np.round(P, 4).tolist(), _ideal_fig(ideal, open_set, n_channels)
...
def generate_and_export(matrix, open_states, n_channels, n_steps, decimate, seed, ckpt_path, progress=gr.Progress()):
    ...
    if two_stage:
        ...
        cide = block_mean(cond_int, factor)
        cide_std = (cide - cnorm["cond_mean"]) / cnorm["cond_std"]
        env_std = generate_long_current(
            coarse, cide_std.astype(np.float32), device,
            progress=lambda f: progress(0.4 * f, desc="Sampling envelope"),
        )
        ...
        cur_std = generate_long_current(
            ...
            progress=lambda f: progress(0.4 + 0.6 * f, desc="Sampling detail"),
        )
    else:
        ...
        cur_std = generate_long_current(
            fine, ide_std, device,
            progress=lambda f: progress(f, desc="Sampling"),
        )
...
with gr.Blocks(title="Ion diffusion trainer") as demo:
    gr.Markdown("## Ion conditional diffusion: live training")
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            n_steps_g = gr.Slider(1000, 2_000_000, value=100_000, step=1000, label="Rows to generate")
    ...
    preview_btn.click(
        preview_ideal,
        inputs=[matrix, open_states, n_channels_g, preview_len],
